...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/medicaltalk-tube/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'dist/medicaltalk-tube/browser'), 'index.html')
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            name: `medicaltalkTube Research ${group?.PageTitle}`,
